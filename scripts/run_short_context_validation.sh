...
for context_length in "${contexts[@]}"; do
  ...
  for plan in "${plans[@]}"; do
    output="results/generated/${suite}-context-${context_length}-${plan}-validation-benchmark.json"
    if [[ -f "${output}" ]]; then
      echo "skip       ${plan} @ ${context_length} | ${output} exists"
      continue
    fi

    echo "validate   ${plan} @ ${context_length} | warmup ${warmup}, iterations ${iterations}"
    "${python_bin}" -m triglu.benchmark \
      ...
      --label context-scaling-validation \
      --output "${output}"
  done
done
